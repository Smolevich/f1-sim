...
test('за секунду набегает 120 шагов независимо от частоты кадров', () => {
  ...
  expect(countAt(60)).toBe(120)
  expect(countAt(144)).toBe(120)
})
...
**Files:**
- Create: `src/track/schema.ts`, `scripts/build-track.ts`, `tracks/monza.json`
...
async function fetchWay(relationId: number): Promise<{ lat: number; lon: number }[]> {
  const query = `[out:json][timeout:60];relation(${relationId});way(r);node(w);out;`
  const res = await fetch(OVERPASS, { method: 'POST', body: query })
  if (!res.ok) throw new Error(`Overpass ответил ${res.status}`)
  const data = await res.json() as { elements: { type: string; lat?: number; lon?: number }[] }
  return data.elements
    .filter((e): e is { type: string; lat: number; lon: number } =>
...
async function main(): Promise<void> {
  ...
  const raw = await fetchWay(circuit.osmRelationId)
  ...
  writeFileSync(`tracks/${circuit.id}.json`, JSON.stringify(track))
  console.log(`записано tracks/${circuit.id}.json`)
}
...
Expected: печатает число точек и длину, длина в пределах 2% от 5793 м, создаётся `tracks/monza.json`

Если OSM-relation оказался не тот (длина не сходится, скрипт выходит с ошибкой) — найти верный id через `https://www.openstreetmap.org/search?query=Autodromo%20Nazionale%20Monza`, поправить `osmRelationId` в `CIRCUITS` и запустить снова. Валидатор для того и стоит перед записью файла.

- [ ] **Step 7: Коммит**

```bash
git add src/track/schema.ts src/track/schema.test.ts scripts/build-track.ts tracks/monza.json
...
import RAPIER from '@dimforge/rapier3d-compat'
import { downforce, drag, type AeroSetup } from './aero'
import { bestGear, rpmFor, wheelTorque } from './drivetrain'
...
export class Vehicle {
  ...
  step(input: CarInput, dt: number): void {
    ...
    WHEEL_OFFSETS.forEach((wheel, i) => {
      ...
      const hit = this.world.castRay(ray, SUSPENSION_REST_M * 1.5, true, undefined, undefined, undefined, this.body)
      if (!hit) return
...
Положить `tracks/monza.json` в `public/tracks/monza.json`, чтобы Vite отдавал его как статику.
