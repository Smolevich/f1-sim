 ...
 */
/**
 * Ближний ряд стоит за отбойниками с запасом на крону.
 *
 * Отбойники физики идут в 9 м от края полотна (BARRIER_OFFSET_M), то есть до
 * 15 м от осевой при ширине 12 м. Крона радиусом 5.6 м при отступе 24 м
 * доходила до 16.8 м — деревья росли прямо в зоне вылета, и вылетевший болид
 * оказывался в лесу вместо гравия.
 */
const BELT_INNER_M = 34
const BELT_DEPTH_M = 70
const BELT_STEP_M = 7
const BELT_ROWS = 6

/** Радиус кроны с учётом наклона: на него ближний ряд отодвигается ещё дальше. */
const CROWN_REACH_M = 7
  ...
  for (let i = 0; i < cl.length; i += stride) {
    ...
    for (const side of [-1, 1]) {
      ...
      for (let r = 0; r < rows; r += 1) {
        ...
        // Проверка по всей осевой: на шпильке соседний виток проходит ближе
        // своего участка, и без неё дерево вырастает у чужого поворота.
        const clear = BELT_INNER_M - CROWN_REACH_M
        if (nearestCentreDistance(track, slot) >= clear) slots.push(slot)
      }
    }
  }
  return slots
