...
test('срезка делает круг невалидным', () => {
  ...
  let completed = null
  let ms = 0
  // Срез — это ПУТЬ вне полотна, а не время: болид проезжает напрямик заметно
  // больше порога. Стоять на месте вне трассы срезом не считается, иначе
  // застрявший в отбойнике терял бы круг ни за что.
  for (let i = 0; i < 40; i++) {
    ms += 100
    // точки идут по кольцу, поэтому между семплами набегает реальное расстояние
    s = updateLap(s, t, at(t, 0.3 + i * 0.007), ms, false).state
  }
...
test('срезка не отмывается чистым остатком круга', () => {
  ...
  let completed = null
  let ms = 0
  // Срез в первом секторе сверх порога, дальше весь круг чисто: набранный путь
  // вне полотна не обнуляется до финиша, иначе аккуратный остаток снимал бы
  // наказание.
  for (let i = 0; i < 40; i++) {
    ms += 100
    s = updateLap(s, t, at(t, 0.05 + i * 0.007), ms, false).state
  }
...
test('счётчик выездов растёт', () => {
  ...
  expect(s.offTrackMetres).toBeGreaterThan(0)
})
